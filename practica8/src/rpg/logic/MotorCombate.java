...
public class MotorCombate {
    ...
    private void InicioCombate() throws SQLException, RecursoNoEncontradoException, DatoInvalidoException, LimiteHabilidadesException {
        ...
        Integer danio_pj1 = obtenerDanio(personaje1);
        Integer danio_pj2 = obtenerDanio(personaje2);

    }

    private Integer obtenerDanio(Personaje pj) throws SQLException {
        ...
        for (Item item : items) {
            if (itemsPJ.containsKey(item.getId())) {
                ...
                Integer danio = item.getBonificador_ataque()*itemsPJ.get(item.getId()); // Daño de la habilidad = bono_ataque del item * cantidad de veces que tiene el item
                dmgTotal_pj += danio;
            }
        }
